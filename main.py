...
async def univariate_data_sum(df:pd.DataFrame,groupby:str,col:str,title:str):
    ...
    return return_data
async def univariatioe_data_avg(df:pd.DataFrame,groupby:str,col:str,title:str):
...
async def get_data():
    ...
    avg_monthly_charge = await univariatioe_data_avg(df,"Contract","MonthlyCharges","Average MonthlyChargesPerContract")
    CustomerTenureByMonth = await univariate_data_cou(df,"tenureInTenth","customerID","CustomerTenureByMonth")
    AvgTotalChargesperTenure= await univariatioe_data_avg(df,"tenureInTenth","TotalCharges","AvgTotalChargesperTenure")
...
async def machineModel(data:dict):    
    ...
    y_pred = knn.predict(custom_df_encoded.values)
